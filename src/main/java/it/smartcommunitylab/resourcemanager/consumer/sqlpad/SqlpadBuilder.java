...
@Component
public class SqlpadBuilder implements ConsumerBuilder {

	@Value("${consumers.sqlpad.enable}")
	private boolean enabled;

	@Value("${consumers.sqlpad.properties}")
	private List<String> properties;

	@Override
	public String getType() {
		return SqlpadConsumer.TYPE;
	}

	@Override
	public String getId() {
		return SqlpadConsumer.ID;
	}
	...
	public Consumer build(Map<String, Serializable> properties) throws NoSuchConsumerException {
		...
		SqlpadConsumer consumer = new SqlpadConsumer(properties);
	...
	public Consumer build(Registration reg) throws NoSuchConsumerException {
		// properties supported
		SqlpadConsumer consumer = new SqlpadConsumer(reg);
		// explicitly call init() since @postconstruct won't work here
		consumer.init();
