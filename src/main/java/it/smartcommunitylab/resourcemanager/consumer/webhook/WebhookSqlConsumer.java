...
public class WebhookSqlConsumer extends Consumer {

    private final static Logger _log = LoggerFactory.getLogger(WebhookSqlConsumer.class);

    public static final String TYPE = SystemKeys.TYPE_SQL;
    public static final String ID = "webhooksql";
    ...
    public WebhookSqlConsumer(Map<String, Serializable> properties) {
        this();
        _properties = properties;
    }

    public WebhookSqlConsumer(Registration reg) {
        this();
        registration = reg;
    ...
    public void addResource(String scopeId, String userId, Resource resource) throws ConsumerException {
        if (checkScope(resource.getScopeId()) && checkTags(resource.getTags())) {
            ...
            try {
                ...
                _client.call("event", resource.getScopeId(), resource.getId(), TYPE, host, port, database, uname,
                        passw);
