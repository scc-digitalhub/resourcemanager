...
@Component
public class MetabaseBuilder implements ConsumerBuilder {

    @Value("${consumers.metabase.enable}")
    private boolean enabled;

    @Value("${consumers.metabase.properties}")
    private List<String> properties;

    @Override
    public String getType() {
        return MetabaseConsumer.TYPE;
    }

    @Override
    public String getId() {
        return MetabaseConsumer.ID;
    }
    ...
    public Consumer build(Map<String, Serializable> properties) throws NoSuchConsumerException {
        ...
        MetabaseConsumer consumer = new MetabaseConsumer(properties);
    ...
    public Consumer build(Registration reg) throws NoSuchConsumerException {
        // properties supported
        MetabaseConsumer consumer = new MetabaseConsumer(reg);
        // explicitly call init() since @postconstruct won't work here
        consumer.init();
